...
final class HighlightsStripView: UIView {
    ...
    init() {
        ...
        addAutoLayout(scroll)
        scroll.addAutoLayout(row)
    ...
    private func makeItem(title: String, diameter: CGFloat, fast: FastScale) -> UIStackView {
        ...
        // Fonts use pre-scaled sizes (fast.text already scales), so skip auto-scaling.
        let letter = UILabel()
        letter.text = String(title.prefix(1))
        letter.textColor = .onAccent
        letter.font = .preScaled(fast.text(24), weight: .bold)
        circle.addAutoLayout(letter)

        let caption = UILabel()
        caption.text = title
        caption.font = .preScaled(fast.text(12))
